...
public class GetExternalSource {
	...
	public List<String> loadParticipantByPosName ( String posName) {
		Service service = new Service();
		...
		{      
		  Call call = (Call) service.createCall();
          call.setTargetEndpointAddress(endpoint);
          call.setOperationName(new QName("http://server.com/", "getEmpByPosName")); //WSDL里面描述的接口名称
          call.addParameter("arg0", org.apache.axis.encoding.XMLType.XSD_STRING,
                  javax.xml.rpc.ParameterMode.IN);//接口的参数
          call.setReturnType(org.apache.axis.encoding.XMLType.XSD_STRING);//设置返回类型  
          result = ( String ) call.invoke( new Object[]{posName});  
          JSONArray json = JSONArray.fromObject(result);
          empIds= JSONArray.toList(json);
		} catch (Exception e) {
			e.printStackTrace();
			return empIds ;
		}
